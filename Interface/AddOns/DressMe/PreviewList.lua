...
function ns:CreatePreviewList(parent)
    ...
    function frame:Update(previewSetup, items, page)
        ...
        itemList = items or {}
        pageCount = math.floor(#itemList / perPage)
        if perPage > 0 and pageCount < #itemList / perPage then pageCount = pageCount + 1 end
        if pageCount < 1 and #itemList > 0 then pageCount = 1 end
