...
do 
    ...
    credits:SetText(L("TITLE_SUB"))
    mainFrame._creditsTitle = credits
    if not L("TITLE_SUB") or L("TITLE_SUB") == "" then
        credits:Hide()
    end
...
do
    ...
    showDressMeButtonCheckBox:HookScript("OnEnter", function(self)
        GameTooltip:SetOwner(self, "ANCHOR_TOPLEFT")
        GameTooltip:ClearLines()
        GameTooltip:AddLine(L("SHOW_DRESSME_TIP1"))
        GameTooltip:AddLine(L("SHOW_DRESSME_TIP2"), 1, 1, 1, 1, true)
        GameTooltip:AddLine(L("SHOW_DRESSME_TIP3"), 1, 1, 1, 1, true)
        GameTooltip:Show()
    end)
    ...
    showDressMeButtonTitle:SetText(L("SHOW_DRESSME_BTN"))
    settingsTab._showBtnTitle = showDressMeButtonTitle
    ...
    local langMenu = CreateFrame("Frame", addon.."LanguageMenu", settingsTab, "UIDropDownMenuTemplate")
    langMenu:SetPoint("LEFT", langTitle, "RIGHT", -10, -2)
    settingsTab._langMenu = langMenu

    local function LanguageLabel(code)
        if code == "es" then return "Español" end
        return "English"
    end

    local function LanguageMenu_OnClick(self, arg1)
        GetSettings().locale = arg1
        ns.SetLocale(arg1)
        UIDropDownMenu_SetText(langMenu, LanguageLabel(arg1))
        UIDropDownMenu_SetSelectedValue(langMenu, arg1)
        PlaySound("gsTitleOptionOK")
    end

    UIDropDownMenu_Initialize(langMenu, function(frame, level, menuList)
        local info = UIDropDownMenu_CreateInfo()
        info.text = "English"
        info.arg1 = "en"
        info.value = "en"
        info.func = LanguageMenu_OnClick
        info.checked = (ns.GetLocale() == "en")
        UIDropDownMenu_AddButton(info)
        ...
        info.value = "es"
        info.func = LanguageMenu_OnClick
        info.checked = (ns.GetLocale() == "es")
        UIDropDownMenu_AddButton(info)
    end)
    UIDropDownMenu_SetWidth(langMenu, 120)
    UIDropDownMenu_SetText(langMenu, "English")
    ...
    local function applySettings(settings)
        ...
        ns.SetLocale(loc)
        UIDropDownMenu_SetText(langMenu, loc == "es" and "Español" or "English")
        UIDropDownMenu_SetSelectedValue(langMenu, loc)
    end
...
function ns.ApplyLocale()
    ...
    if mainFrame._titleMain then mainFrame._titleMain:SetText(Ls("TITLE_MAIN")) end
    if mainFrame._creditsTitle then mainFrame._creditsTitle:SetText(Ls("TITLE_SUB")) end
    ...
    if st then
        ...
        if st._showBtnTitle then st._showBtnTitle:SetText(Ls("SHOW_DRESSME_BTN")) end
        if st._langTitle then st._langTitle:SetText(Ls("LANGUAGE")) end
        if st._creditsLbl then st._creditsLbl:SetText(Ls("CREDITS")) end
        if st._langMenu then
            local loc = ns.GetLocale()
            UIDropDownMenu_SetText(st._langMenu, loc == "es" and "Español" or "English")
            UIDropDownMenu_SetSelectedValue(st._langMenu, loc)
        end
    end
...
do
    ...
    local btn = CreateFrame("Button", "DressMeMinimapButton", Minimap)
    btn:SetSize(32, 32)
    ...
    btn:SetScript("OnDragStop", function(self)
        ...
        if type(DressMeSettings) == "table" then
            DressMeSettings.minimapAngle = self.angle
            DressMeSettings.minimapHide = false
        end
    ...
    btn:SetScript("OnEnter", function(self)
        ...
        GameTooltip:AddLine("github.com/GetLocalPlayer/DressMe", 0.5, 0.5, 0.8)
    ...
    loader:SetScript("OnEvent", function()
        if type(DressMeSettings) == "table" then
            if DressMeSettings.minimapAngle then
                btn.angle = DressMeSettings.minimapAngle
                UpdateMinimapButtonPosition(btn, btn.angle)
            end
            if DressMeSettings.minimapHide then
                btn:Hide()
            else
                btn:Show()
            end
    ...
    SLASH_DRESSMEMINIMAP1 = "/dressmeminimap"
    SlashCmdList["DRESSMEMINIMAP"] = function()
        if btn:IsShown() then
            btn:Hide()
            if type(DressMeSettings) == "table" then DressMeSettings.minimapHide = true end
            DEFAULT_CHAT_FRAME:AddMessage("|cff00ff00[DressMe]|r Boton del minimapa oculto. /dressmeminimap para mostrarlo.")
        else
            btn:Show()
            if type(DressMeSettings) == "table" then DressMeSettings.minimapHide = false end
            DEFAULT_CHAT_FRAME:AddMessage("|cff00ff00[DressMe]|r Boton del minimapa visible.")
        end
    end
end
